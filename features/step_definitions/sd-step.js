...
module.exports = function server() {
    ...
    this.Then(/^(.*) package is available via sd-step$/, { timeout: TIMEOUT }, (pkg) => {
        request({
            uri: `${this.instance}/${this.namespace}/builds/${this.buildId}`,
            method: 'GET',
            json: true
        }).then((response) => {
            Assert.equal(response.statusCode, 200);
    ...
    // for second pass
    this.Then(/^(.*) package is available via sd-step with specified version (.*)$/, { timeout: TIMEOUT }, (pkg, version) => null);
    // for second pass
    this.Then(/^(.*) package is available via sd-step without installation\/download time$/, { timeout: TIMEOUT }, (pkg) => null);
};
